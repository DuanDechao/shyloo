...
{
	pyscript::PyGC::incTracing("Vector3");
}
...
ScriptVector3::~ScriptVector3()
{
	...
	_pyVector3ChangedCallback = NULL;
	pyscript::PyGC::decTracing("Vector3");
}
...
PyObject* ScriptVector3::tp_repr()
{
	...
	for(int i=0; i < VECTOR_SIZE; ++i)
	{
		if (i > 0)
			strcat(str, ", ");
		SafeSprintf(str + strlen(str), 128, "%f", v[i]);
	}
...
{ 
	return PyFloat_FromDouble(SLVec3Length(&getVector())); 
}

//-------------------------------------------------------------------------------------
PyObject* ScriptVector3::pyGetVectorLengthSquared()
{ 
	return PyFloat_FromDouble(SLVec3LengthSq(&getVector()));
}
...
PyObject* ScriptVector3::__reduce_ex__(PyObject* self, PyObject* protocol)
{
	...
	PyObject* unpickleMethod = pyscript::Pickler::getUnpickleFunc("Vector3");
...
PyObject* ScriptVector3::__unpickle__(PyObject* self, PyObject* args)
{
	...
	if(size != VECTOR_SIZE)
	{
		printf("ScriptVector3::__unpickle__: args is error! size != 3");
		S_Return;
	}

	if(!PyArg_ParseTuple(args, "fff", &x, &y, &z))
	{
		printf("ScriptVector3::__unpickle__: args is error!");
		S_Return;
	}
...
void ScriptVector3::onInstallScript(PyObject* mod)
{
	...
	pyscript::Pickler::registerUnpickleFunc(pyFunc, "Vector3");
	Py_DECREF(pyFunc);
...
PyObject* ScriptVector3::__py_pyFlatDistTo(PyObject* self, PyObject* args)
{
	...
	Vector3 v1;
	convertPyObjectToVector3(v1, pyVal);
	return PyFloat_FromDouble(SLVec3CalcVec2Length(v, v1));
}
...
PyObject* ScriptVector3::__py_pyDistTo(PyObject* self, PyObject* args)
{
	...
	Vector3 rv = (v - v1);
	return PyFloat_FromDouble(SLVec3Length(&rv)); //计算长度并返回
}
...
PyObject* ScriptVector3::__py_pyDistSqrTo(PyObject* self, PyObject* args)
{
	...
	Vector3 rv = (v - v1);
	return PyFloat_FromDouble(SLVec3LengthSq(&rv)); //计算点乘并返回
}
...
PyObject* ScriptVector3::__py_pyDot(PyObject* self, PyObject* args)
{
	...
	ScriptVector3* sv = static_cast<ScriptVector3*>(self);
	float result = SLVec3Dot(const_cast<Vector3*>(&sv->getVector()), const_cast<Vector3*>(&v->getVector()));
	Py_DECREF(v);
	return PyFloat_FromDouble(result);
}
