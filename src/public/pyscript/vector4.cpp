...
isCopy_(true)
{
	pyscript::PyGC::incTracing("Vector4");
}
...
ScriptVector4::~ScriptVector4()
{
	...
	pyscript::PyGC::decTracing("Vector4");
}
...
{ 
	return PyFloat_FromDouble(SLVec4Length(&getVector())); 
}

//-------------------------------------------------------------------------------------
PyObject* ScriptVector4::pyGetVectorLengthSquared()
{ 
	return PyFloat_FromDouble(SLVec4LengthSq(&getVector()));
}
...
PyObject* ScriptVector4::__reduce_ex__(PyObject* self, PyObject* protocol)
{
	...
	PyObject* unpickleMethod = pyscript::Pickler::getUnpickleFunc("Vector4");
...
PyObject* ScriptVector4::__unpickle__(PyObject* self, PyObject* args)
{
	...
	if(size != VECTOR_SIZE)
	{
		printf("ScriptVector4::__unpickle__: args is error! size != 4");
		S_Return;
	}

	if(!PyArg_ParseTuple(args, "ffff", &x, &y, &z, &w))
	{
		printf("ScriptVector4::__unpickle__: args is error!");
		S_Return;
	}
...
void ScriptVector4::onInstallScript(PyObject* mod)
{
	...
	pyscript::Pickler::registerUnpickleFunc(pyFunc, "Vector4");
	Py_DECREF(pyFunc);
...
PyObject* ScriptVector4::__py_pyDistTo(PyObject* self, PyObject* args)
{
	...
	Vector4 rv = (v - v1);
	return PyFloat_FromDouble(SLVec4Length(&rv)); //计算长度并返回
}
...
PyObject* ScriptVector4::__py_pyDistSqrTo(PyObject* self, PyObject* args)
{
	...
	Vector4 rv = (v - v1);
	return PyFloat_FromDouble(SLVec4LengthSq(&rv)); //计算点乘并返回
}
...
PyObject* ScriptVector4::__py_pyDot(PyObject* self, PyObject* args)
{
	...
	ScriptVector4* sv = static_cast<ScriptVector4*>(self);
	float result = SLVec4Dot(const_cast<Vector4*>(&sv->getVector()), const_cast<Vector4*>(&v->getVector()));
	Py_DECREF(v);
