...
bool PyGC::initialize(void)
{
	...
	if(gcModule)
	{
		...
		static PyMethodDef __pymethod_debugTracing = {"debugTracing", (PyCFunction)__py_debugTracing, METH_VARARGS, NULL};
		if(PyModule_AddObject(gcModule, "debugTracing", PyCFunction_New(&__pymethod_debugTracing, 0)) != 0){
			if (PyErr_Occurred()){
				PyErr_PrintEx(0);
			}
			 printf("append debugTracing to pyscript is error!\n");
		}
