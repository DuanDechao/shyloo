...
PyObject* ScriptDefModule::createPyObject(const uint64 entityId, bool isInitializeScript, bool initProperty){
    ...
    IObject* object = CREATE_OBJECT_BYID(SLMODULE(ObjectMgr), _moduleName.c_str(), entityId);
    ScriptObject* pyScriptObject = static_cast<ScriptObject*>(pyObject);
    pyScriptObject->setInnerObject(object);
    
    if(isInitializeScript)
        pyObject->initializeScript();
