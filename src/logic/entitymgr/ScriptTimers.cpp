...
int64 ScriptTimers::addTimer(sl::api::IKernel* pKernel, PyObject* entity, int64 period, int64 interval, const char* callbackName, PyObject* userData){
	ScriptTimer* newTimer = ScriptTimer::create(entity, _lastTimerId++, callbackName, userData);
	_timers[newTimer->getTimerId()] = newTimer;
	START_TIMER(newTimer, 0, period, interval);
