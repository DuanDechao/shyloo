...
bool Starter::launched(sl::api::IKernel * pKernel){
	...
	RGS_EVENT_HANDLER(SLMODULE(EventEngine), logic_event::EVENT_PRE_SHUTDOWN, Starter::preShutDown);
...
void Starter::onTime(sl::api::IKernel* pKernel, int64 timetick){
	sl::BStream<256> args;
	SLMODULE(Harbor)->broadcast(NodeProtocol::MASTER_MSG_SERVER_STARTED, args.out());
}
...
void Starter::startNode(sl::api::IKernel* pKernel, int32 nodeType, int32 nodeId){
	...
	if (slave != game::NODE_INVALID_ID){
		...
		SLMODULE(Harbor)->send(NodeType::SLAVE, slave, NodeProtocol::MASTER_MSG_START_NODE, args.out());
...
void Starter::preShutDown(sl::api::IKernel* pKernel, const void* context, const int32 size){
	...
	sl::BStream<32> args;
	SLMODULE(Harbor)->broadcast(NodeType::SLAVE, NodeProtocol::MASTER_MSG_STOP_NODES, args.out());
}
