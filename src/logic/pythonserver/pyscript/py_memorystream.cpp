...
readonly_(readonly)
{
	...
	pyscript::PyGC::incTracing("MemoryStream");
}
...
PyObject* PyMemoryStream::__py_reduce_ex__(PyObject* self, PyObject* protocol)
{
	...
	PyObject* unpickleMethod = pyscript::Pickler::getUnpickleFunc("MemoryStream");
...
PyObject* PyMemoryStream::__unpickle__(PyObject* self, PyObject* args)
{
	...
	if (size != 3)
	{
		printf("PyMemoryStream::__unpickle__: args is wrong! (size != 3)");
		S_Return;
	}
	...
	if (pybytes == NULL)
	{
		printf("PyMemoryStream::__unpickle__: args is wrong!");
		S_Return;
	}
...
void PyMemoryStream::onInstallScript(PyObject* mod)
{
	...
	pyscript::Pickler::registerUnpickleFunc(pyFunc, "MemoryStream");
	Py_DECREF(pyFunc);
}
...
void PyMemoryStream::addToStream(MemoryStream* mstream)
{
	...
	if(size > 0)
	{
		int32 rpos = stream().rpos(), wpos = stream().wpos();
		(*mstream) << rpos;
		(*mstream) << wpos;
...
PyObject* PyMemoryStream::__py_append(PyObject* self, PyObject* args, PyObject* kwargs)
{
	...
	else if(strcmp(type, "STRING") == 0)
	{
		...
		char* s = sl::CStringUtils::wchar2char(ws);
		PyMem_Free(ws);
