...
bool Pickler::initialize(void)
{
	...
	if(cPickleModule)
	{
		...
		if (!picklerMethod_)
		{
			printf("Pickler::initialize: get dumps is error!\n");
			PyErr_PrintEx(0);
		}
		...
		if(!unPicklerMethod_)
		{
			printf("Pickler::init: get loads is error!\n");
			PyErr_PrintEx(0);
		}
	...
	{
		printf("PyGC::initialize: can't import pickle!\n");
		PyErr_PrintEx(0);
	}
	...
	if(isInit)
	{
		...
		{  
			 ...
			 "_upf",  
			 "This module is created by shyloo!",  
			 -1,  
			 NULL  
		};  
...
std::string Pickler::pickle(PyObject* pyobj, int8 protocol)
{
	...
	PyObject* pyRet = PyObject_CallFunction(picklerMethod_, 
		const_cast<char*>("(O)"), pyobj, protocol);
...
PyObject* Pickler::unpickle(const std::string& str)
{
	...
	if (!pyRet)
	{
		...
		{
			if ((uint8)str[i] >= 32 && (uint8)str[i] <= 126)
				buff += str[i];
			//else
			//	buff += fmt::format("\\x{:02x}", (uchar)str[i]);
		}
		SLASSERT(false, "failed to unpickle");
		printf("Pickler::unpickle: failed to unpickle[%s] len=%d.\n",
			buff.c_str(), str.length());
	}
