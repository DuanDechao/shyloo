...
isCopy_(true)
{
	pyscript::PyGC::incTracing("Vector2");
}
...
ScriptVector2::~ScriptVector2()
{
	...
	pyscript::PyGC::decTracing("Vector2");
}
...
{ 
	return PyFloat_FromDouble(SLVec2Length(&getVector())); 
}

//-------------------------------------------------------------------------------------
PyObject* ScriptVector2::pyGetVectorLengthSquared()
{ 
	return PyFloat_FromDouble(SLVec2LengthSq(&getVector()));
}
...
PyObject* ScriptVector2::__reduce_ex__(PyObject* self, PyObject* protocol)
{
	...
	PyObject* unpickleMethod = pyscript::Pickler::getUnpickleFunc("Vector2");
...
PyObject* ScriptVector2::__unpickle__(PyObject* self, PyObject* args)
{
	...
	if(size != VECTOR_SIZE)
	{
		printf("ScriptVector2::__unpickle__: args is error! size != 2");
		S_Return;
	}

	if(!PyArg_ParseTuple(args, "ff", &x, &y))
	{
		printf("ScriptVector2::__unpickle__: args is error!");
		S_Return;
	}
...
void ScriptVector2::onInstallScript(PyObject* mod)
{
	...
	pyscript::Pickler::registerUnpickleFunc(pyFunc, "Vector2");
	Py_DECREF(pyFunc);
...
PyObject* ScriptVector2::__py_pyDistTo(PyObject* self, PyObject* args)
{
	...
	Vector2 rv = (v - v1);
	return PyFloat_FromDouble(SLVec2Length(&rv)); //计算长度并返回
}
...
PyObject* ScriptVector2::__py_pyDistSqrTo(PyObject* self, PyObject* args)
{
	...
	Vector2 rv = (v - v1);
	return PyFloat_FromDouble(SLVec2LengthSq(&rv)); //计算点乘并返回
}
...
PyObject* ScriptVector2::__py_pyDot(PyObject* self, PyObject* args)
{
	...
	ScriptVector2* sv = static_cast<ScriptVector2*>(self);
	float result = SLVec2Dot(const_cast<Vector2*>(&sv->getVector()), const_cast<Vector2*>(&v->getVector()));
	Py_DECREF(v);
	return PyFloat_FromDouble(result);
}
