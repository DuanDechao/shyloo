...
namespace sl
{
	...
	{
	public:
		CNullMutex& operator= (const CNullMutex& SL_UNUSED(obj)) {return *this;}
	...
	*/
	...
	public:
		CLogT()
		{
			...
			sl_snprintf(SL_STRSIZE(m_szLogPath), "%s", DEFAULT_LOGNAME);
			m_szLogSuffix[0] = 0;
			m_szLine[0] = 0;
			m_uiLine = 0;
			m_pFile = 0;
	...
	public:
			...
			int iMaxLogNum = SL_DEFAULT_LOGNUM)
		{
			...
			if(m_eLogType == ENamed)
			{
				...
				sl_snprintf(SL_STRSIZE(m_szLogPath), "%s%s",
			...
			else
			{
				...
				sl_snprintf(SL_STRSIZE(m_szLogPath), "%s", str.c_str());
				sl_snprintf(SL_STRSIZE(m_szLogSuffix), "%s", (szLogName == NULL ? DEFAULT_SUFFIX : szLogName));
			}
		}
	...
	#define DO_LOG(filter, format) \
		...
		int VLog(ELogFilter filter, const char* szFormat, va_list ap)
		{
			...
			if(m_eLogType == ENamed)
			{
				sl_snprintf(SL_STRSIZE(szFilePath), "%s", m_szLogPath);
			}
			else
			{
				switch(m_eLogType)
				{
				case EDaily:
					stNow.Format(SL_STRSIZE(szTimeBuf), "%Y%m%d");
					break;
				case EMonth:
					stNow.Format(SL_STRSIZE(szTimeBuf), "%Y%m");
					break;
				case EHour:
					stNow.Format(SL_STRSIZE(szTimeBuf), "%Y%m%d_%H");
					break;
				default:
					stNow.Format(SL_STRSIZE(szTimeBuf), "%Y%m%d");
					break;
				...
				if(m_szLogPath[0] != 0)
				{
					sl_snprintf(SL_STRSIZE(szFilePath), "%s%s%s", m_szLogPath, szTimeBuf, m_szLogSuffix);
				}
				else
				{
					sl_snprintf(SL_STRSIZE(szFilePath), "%s%s", szTimeBuf, m_szLogSuffix);
				}
			}
			...
			if(m_uiFormat & ETime)
			{
				...
				iRet = sl_snprintf(pLine, iSize, "[%s.%06u]", szTimeBuf, stNowTime.tv_usec);
				pLine += iRet;
				iSize -= iRet;
			}
			...
			{
				iRet = sl_snprintf(pLine, iSize, "[%d]", static_cast<int>(GetNowProcessId()));
				pLine += iRet;
				iSize -= iRet;
			}
			...
			if(m_uiFormat & EType)
			{
				iRet = sl_snprintf(pLine, iSize, "[%s]", GetFilterDesc(filter));
				pLine += iRet;
				iSize -= iRet;
			}

			iRet  = sl_vsnprintf(pLine, iSize, szFormat, ap);
			pLine += iRet;
			iSize -= iRet;

			if(m_uiFormat & EFileLine)
			{
				iRet = sl_snprintf(pLine, iSize, " (%s:%d)",m_pFile, m_uiLine);
				pLine += iRet;
				iSize -= iRet;
			}
			...
			if( !(m_uiFormat & ENoFile) )
			{
				...
				FILE* pstFile = sl_fopen(szFilePath, "a+");
				umask(iMode);
			#else
				FILE* pstFile = sl_fopen(szFilePath, "a+");
			#endif
		...
		*/
		int ShiftFiles(const char* pszFilePath, int iMaxLogSize, int iMaxLogNum)
		{
			...
			struct stat	stStat;
#endif
			if(sl_stat(pszFilePath, &stStat) < 0)
			{
				return -3; ///获取日志文件信息失败
			}
			...
			sl_snprintf(SL_STRSIZE(szTempBuf), "%s.%d", pszFilePath, iMaxLogNum-1);
			if(sl_access(szTempBuf, 0) == 0)
			{
				if(sl_remove(szTempBuf) < 0)
				{
			...
			for(int i = iMaxLogNum - 2; i >= 0; --i)
			{
				if(i == 0)
				{
					sl_snprintf(SL_STRSIZE(szTempBuf), "%s", pszFilePath);
				}
				else
				{
					sl_snprintf(SL_STRSIZE(szTempBuf), "%s.%d", pszFilePath, i);
				}

				if(sl_access(szTempBuf, 0) == 0)
				{
					sl_snprintf(SL_STRSIZE(szNewFile), "%s.%d", pszFilePath, i+1);
					if(sl_rename(szTempBuf, szNewFile) < 0)
					{
						return -5;  ///< 重命名日志文件失败
					}
				}
