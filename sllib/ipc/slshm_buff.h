...
#ifndef __SL_SHM_BUFF_H__
#define __SL_SHM_BUFF_H__
#include "slshm.h"
...
namespace sl
{
	...
	public:
		...
		{
			sl_snprintf(m_szBuffName, ES_BUFFNAME_MAX, "%s", pszName);
			SL_INFO("alloc buff(%s) node name (%s)", pszName, m_szBuffName);
		}
		...
		int MakeBuff(char* pszBuff, bool bResetShm)
		{
			...
			{
				SL_ERROR("shm buff(%s) do size(%d) do not have init Func!", m_szBuffName, m_uiBuffSize);
				return -1;
			}
		...
		{
			sl_snprintf(m_szBuffName, ES_BUFFNAME_MAX, "%s", obj.m_szBuffName);
	...
	private:
		...
		int InsertBuff(const CShmBuffNode& stNode)
		{
			...
			{
				//共享内存buff已满
				SL_ERROR("shm buff list is full!");
				return 1;
			}
		...
		int BuffAttachShm()
		{
			...
			for(unsigned int i = 0; i < m_uiShmSize; ++i)
			{
				...
				if(iRet)
				{
					SL_ERROR("make buff(%s) err(%d)!", m_stShmArray[i].GetBuffName(), iRet);
					return iRet;
				}
				SL_INFO("alloc shm Buff(%s) at addr(%p) size(%u)",
	...
	public:
		...
		*/
		int RegisterBuff(const char* stBuffName, unsigned int uiBuffSize, PInitBuffFunc pFunc)
		{
			if(pFunc == NULL)
			{
				SL_ERROR("register buff(%s) is no init func!", stBuffName);
				return -1;
			}
			...
			{
				//所需内存已经越界
				SL_ERROR("need size(%lu) is over load!", m_ui64BuffSize);
				return -1;
			}
			SL_INFO("buff(%s) size(%u) register into shm size count(%lu)", stBuffName, uiBuffSize, m_ui64BuffSize);
			return 0;
		}
		...
		int CreateBuff(key_t uikey)
		{
			...
			if(iRet)
			{
				SL_ERROR("buff attach shm fails(%d)!", iRet);
			}
			return iRet;
		}
#endif
